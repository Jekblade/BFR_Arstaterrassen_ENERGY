...
class BuildingEnergySimulation:
    def __init__(self, tes_volume_m3, heat_pump_cop, pv_panel_capacity_kwp, hp_min_electrical_input_kw=5, acdc_loss_factor=0.03):
        ...
        self.pv_config = {
            'South_27': {'azimuth': 185, 'slope': 27, 'count': 300}, # 546 
            'South_23': {'azimuth': 185, 'slope': 23, 'count': 0}, # 320
    ...
    def simulate_hour(self, timestamp, hot_water_demand_kwh_hour, dni_wh_per_m2_hour, dhi_wh_per_m2_hour, electricity_price_sek_per_kwh):
        ...
        # Use grid electricity if we still have unmet demand or need to maintain minimum TES level
        grid_to_hp_kwh = 0
        thermal_from_grid = 0
        ...
        # Total thermal needed from grid
    ...
    def plot_weekly_performance(self, week_number):
        ...
        ax1.plot(hours_in_week, weekly_data['hot_water_demand_kwh'] / 2.5, label='Hot Water Demand (kWh elec)', color='blue')
        ax1.plot(hours_in_week, weekly_data['pv_generation_kw_net'], label='Net PV Generation (kWh elec)', color='green')
        ax1.plot(hours_in_week, weekly_data['grid_total_usage_kwh_hour'], label='Grid to HP (kWh elec)', color='red')
        ax1.plot(hours_in_week, weekly_data['hp_electrical_consumed_kwh'], label='HP Electrical Input (kWh)', color='purple', linestyle=':')
...
# Main function updated for DHI input
def run_full_simulation_process(demand_file, dni_dhi_file, price_file, tes_volume_m3, heat_pump_cop, 
                                pv_panel_capacity_kwp, hp_min_electrical_input_kw, plot_weekly=False, week_number=None):
